...
const proxy = (prefix, target) => createProxyMiddleware({
    ...
    on: {
        error: (err, req, res) => {
            console.error(`[Proxy Error] ${req.method} ${req.originalUrl} -> ${target}: [${err.code}] ${err.message}`);
            if (!res.headersSent) {
                // DEBUG TẠM: dùng 200 thay vì 502/503/504 vì Render tự chèn trang lỗi
                // HTML thương hiệu riêng, đè lên response gốc, khi thấy các status đó —
                // khiến không đọc được message lỗi thật qua DevTools. Trả lại đúng mã
                // lỗi sau khi debug xong.
                res.writeHead(200, { 'Content-Type': 'application/json' });
            }
            res.end(JSON.stringify({
                success: false,
                message: 'Proxy error (DEBUG: status tạm là 200)',
                data: { target, code: err.code || null, error: err.message },
            }));
        },
    },
});
