...
// Cafe thuần (không bán đồ ăn) chỉ hợp cho việc nghỉ chân/uống nước, không phải bữa chính
// nên thời gian tham quan ngắn hơn quán ăn thật — dùng chung điều kiện với pickMeal ở
// planning.engine.js để 2 nơi luôn đồng nhất cách phân biệt cafe/quán ăn.
const isCafeOnly = (kinds) => /cafes/.test(kinds) && !/restaurants|fast_food/.test(kinds);

// Địa điểm "xem nhanh" (cầu, tượng đài, điểm ngắm cảnh...) không cần 90p mặc định như
// các điểm tham quan khác — khách chỉ dừng chụp ảnh/ngắm cảnh vài chục phút.
const QUICK_VISIT_KINDS = /bridges|view_points|monuments_and_memorials|statues|fountains|gates|towers/;

const estimateVisitMinutes = (category, kinds) => {
  if (category === "food") return isCafeOnly(kinds) ? 30 : 60;
  if (QUICK_VISIT_KINDS.test(kinds)) return 20;
